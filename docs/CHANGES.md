...
21. 获取音频候选列表中的已确认音频增加右侧播放/暂停按钮；预览仅播放资源，不改变点击文件名后的保存流程，关闭结果列表或打开新列表时会停止预览。
22. 获取音频结果列表的底部增加 56px 导航安全留白，并叠加系统底部安全区，避免最后一项与 Android 的 Home/手势导航区域重叠。
23. 图片预览支持双指缩放、拖动查看、双击缩放与滚轮缩放；关闭时重置缩放状态。应用圆角图标改为按 drawable-nodpi/make_round_png.json 规则从 drawable/icon.png 在构建时生成。
