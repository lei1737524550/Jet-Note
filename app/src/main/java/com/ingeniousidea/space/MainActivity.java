...
public class MainActivity extends Activity {
    ...
    private IntroGuideController introGuideController;
    private boolean introGuideChecked;
    ...
    private boolean introGuidePreloadReady;
    ...
    private void maybeFinishNativeStartupSplash() {
        if (!nativeStartupSplashMinimumElapsed || !frontendIsReady || !introGuidePreloadReady || nativeStartupSplashOverlay == null) return;
        root.removeView(nativeStartupSplashOverlay);
        nativeStartupSplashOverlay = null;
        if (!introGuideChecked) {
            introGuideChecked = true;
            if (introGuideController == null) introGuideController = new IntroGuideController(this, root);
            introGuideController.showFirstLaunchIfAvailable();
        }
    }
    ...
    public void showIntroTutorial() {
        runOnUiThread(() -> {
            if (introGuideController == null) introGuideController = new IntroGuideController(this, root);
            introGuideController.preloadAsync(() -> introGuideController.showManual());
        });
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Use the startup animation as a real preload window for the entrance guide.
        // It stays visible until every guide image for the current language has been decoded,
        // so tapping Next never has to wait for disk I/O or BitmapFactory.
        introGuidePreloadReady = false;
        introGuideController = new IntroGuideController(this, root);
        introGuideController.preloadAsync(() -> {
            introGuidePreloadReady = true;
            maybeFinishNativeStartupSplash();
        });

        // Give first-entry preload a little more visible breathing room. The splash may
        // remain longer automatically if decoding has not completed yet.
        nativeStartupSplashMinimumDurationMs = Math.max(nativeStartupSplashMinimumDurationMs, 1500L);
