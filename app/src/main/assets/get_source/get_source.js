(() => {
  ...
  const showResults = () => {
    ...
    ['image', 'audio', 'video'].forEach(id => {
      ...
      // Hold a resource-type icon for 300 ms to download every matched resource
      // in that category. A normal tap still only switches the visible category.
      let batchTimer = null;
      let batchTriggered = false;
      const cancelBatchHold = () => { if (batchTimer) clearTimeout(batchTimer); batchTimer = null; };
      const startBatchHold = event => {
        cancelBatchHold();
        batchTriggered = false;
        batchTimer = setTimeout(() => {
          batchTimer = null;
          batchTriggered = true;
          const urls = sortedFor(type).filter(row => row.matched).map(row => row.entry.url);
          if (!urls.length) {
            alert(ds('noBatchResources', 'No captured ' + type.label.toLowerCase() + ' resources to download.'));
            return;
          }
          location.href = 'jetnote-save://batch?type=' + encodeURIComponent(id) + '&urls=' + encodeURIComponent(JSON.stringify(urls));
        }, 300);
      };
      const finishBatchHold = event => {
        cancelBatchHold();
        if (batchTriggered) {
          event.preventDefault();
          event.stopPropagation();
      ...
      button.addEventListener('pointerdown', startBatchHold, {passive:true});
      button.addEventListener('pointerup', finishBatchHold, {passive:false});
      button.addEventListener('pointercancel', cancelBatchHold, {passive:true});
      button.addEventListener('pointerleave', cancelBatchHold, {passive:true});
