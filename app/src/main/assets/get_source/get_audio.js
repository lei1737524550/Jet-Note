(() => {
  ...
  const validateAudio = url => new Promise(resolve => {
    let settled = false;
    ...
    audio.addEventListener('loadedmetadata', () => {
      ...
      finish(audio.readyState >= 1 && (Number.isFinite(duration) ? duration >= 0 : duration === Infinity));
    }, {once:true});
    audio.addEventListener('canplay', () => finish(true), {once:true});
    audio.addEventListener('error', () => finish(false), {once:true});
    try { audio.src = url; audio.load(); } catch (_) { finish(false); }
  });
  ...
  source.registerType({
    ...
    matches(url) {
      // This is candidate discovery only. validate() is authoritative.
    ...
    render({panel, list, row, index, bindLongPressSave, colors, ds}) {
      ...
      bindLongPressSave(button, row.entry.url, source.canAddToPost?.() ? () => { location.href = 'jetnote-add://audio?url=' + encodeURIComponent(row.entry.url); } : null);
      ...
      preview.onclick = event => {
        ...
        panel.__jetNotePlayingAudio = audio; panel.__jetNotePlayingButton = preview;
