(() => {
  ...
  const strings = window.JET_NOTE_DICTIONARY_STRINGS || {};
  const ds = (key, fallback) => typeof strings[key] === 'string' ? strings[key] : fallback;
  ...
  if (!found.length) {
    alert(ds('noAudio', 'No audio was found. Play the pronunciation once, then tap the audio button.'));
  ...
  const stopPreview = () => {
    ...
    if (playingButton) {
      ...
      playingButton.setAttribute('aria-label', ds('playAudio', 'Play audio'));
    }
  ...
  close.textContent = ds('close', 'Close');
  ...
  hint.textContent = ds('foundCandidates', 'Found {count} candidate resource(s). Audio resources are listed first.').replace('{count}', String(found.length));
  ...
  found.forEach((entry, index) => {
    ...
    link.textContent =
      ...
      (
        entry.isAudio
          ? ''
          : (
              ds('nonAudio', ' (non-audio resource)')
            )
      );
    ...
    play.setAttribute('aria-label', ds('playAudio', 'Play audio'));
    ...
    const reportPreviewFailure = () => {
      ...
      alert(ds('previewFailed', 'Audio preview failed. Try saving the file and playing it locally.'));
    };
      ...
      preview
        ...
        .then(() => {
          if (playingAudio === preview) {
            ...
            play.setAttribute('aria-label', ds('pauseAudio', 'Pause audio'));
          }
        })
