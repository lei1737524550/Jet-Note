...
function setPostStarState(post, state) {
  ...
  if (normalized === 'star' && previous !== 'star') {
    post.favoritedAt = new Date().toISOString();
    delete post.starredAt;
  }
...
function getMainPosts()
{
  ...
  const star = display.star_post
    ? visible.filter(post => getPostStarState(post) === 'star').sort((a, b) => {
        const aStarred = Date.parse(a?.favoritedAt || a?.starredAt || '');
        const bStarred = Date.parse(b?.favoritedAt || b?.starredAt || '');
        ...
        // Legacy favorites may not have favoritedAt. Keep their old deterministic
