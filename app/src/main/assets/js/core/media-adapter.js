...
const NativeMedia = {
  ...
  url(meta) { return meta?.path && /^media\/[A-Za-z0-9_.-]+$/.test(meta.path) ? 'https://jetnote.local/' + meta.path : ''; },
  isImage(src) { return /^data:image\//i.test(src || '') || /^https:\/\/jetnote\.local\/media\/[A-Za-z0-9_.-]+$/.test(src || ''); },
  async storeBlob(meta, blob) {
    ...
    try {
      ...
      return {...meta,...result,path};
    } catch(error) {JetNoteNative.cancelMedia(token);throw error;}
  },
  async ensure(meta) {
    if (this.url(meta)) return meta;
    const stored=await EntryStore.media(meta.id);
    if (this.url(stored)) return {...meta,...stored};
  ...
  async image(source) {
    if(source.startsWith('data:image/')) {
      ...
      return this.storeBlob({id:'image-'+hash,type:'image',mimeType:match[1],originalName:null,sha256:hash,size:bytes.length},new Blob([bytes],{type:match[1]}));
    }
...
function mediaExtension(mime) {return ({'audio/mpeg':'mp3','audio/wav':'wav','audio/x-wav':'wav','audio/ogg':'ogg','audio/mp4':'m4a','audio/aac':'aac','audio/flac':'flac','image/jpeg':'jpg','image/png':'png','image/gif':'gif','image/webp':'webp','video/mp4':'mp4'}[mime]||'bin');}
const nativePickResolvers=new Map();
window.JetNoteNativeCallbacks={
  onAttachmentsPicked(id,items){const p=nativePickResolvers.get(id);nativePickResolvers.delete(id);p?.resolve(items);},
  onAttachmentPickCancelled(id){const p=nativePickResolvers.get(id);nativePickResolvers.delete(id);p?.resolve([]);},
  onAttachmentPickError(id,message){const p=nativePickResolvers.get(id);nativePickResolvers.delete(id);p?.reject(Error(message));}
};
function pickNativeAttachments(type) {
  return new Promise((resolve,reject)=>{const id=entryUuid();nativePickResolvers.set(id,{resolve,reject});try{JetNoteNative.pickAttachments(id,type,true);}catch(e){nativePickResolvers.delete(id);reject(e);}});
}
async function pickEntryAudio(kind) {
  if(!NativeMedia.available()){document.getElementById(kind+'AudioPicker').click();return;}
...
async function pickEntryMedia(kind,type) {
  ...
  try {
    ...
    for(const meta of accepted){draftAttachments[kind].push(meta);draftMedia[kind].set(meta.id,meta);}
    if(type==='image'){postDraftImages.push(...accepted.map(x=>NativeMedia.url(x)));renderPostImagePreview();}
    renderAudioDraft(kind);
  }catch(error){alert(error.message);}finally{button.disabled=false;}
}
function pruneDraftImages(kind,sources){const used=new Set(sources);draftAttachments.post=draftAttachments.post.filter(x=>x.type!=='image'||used.has(NativeMedia.url(x)));}
